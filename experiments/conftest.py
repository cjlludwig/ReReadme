...
@pytest.fixture(scope="session")
def generated_readme_rereadme():
    """Run rereadme against the rereadme dataset and return generated content."""
    if not os.environ.get("OPENAI_API_KEY"):
        pytest.fail("OPENAI_API_KEY environment variable is required")
    ...
    os.makedirs(results_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%dT%H%M%S")
    backup_path = os.path.join(results_dir, f"rereadme-{timestamp}.md")
    shutil.copy2(output_path, backup_path)
...
def golden_readme_rereadme():
    ...
    if os.path.exists(REREADME_GOLDEN_PATH):
        with open(REREADME_GOLDEN_PATH, "r") as f:
            return f.read()
    return None
